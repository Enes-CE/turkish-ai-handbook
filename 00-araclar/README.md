# 00 — Araçlar

Bu bölümdeki konular tek başına bir "eğitim" değil, diğer bütün konuların ön koşulu.
Python da öğrensen derin öğrenme de çalışsan, bir noktada buradaki araçlara ihtiyacın olacak.

| Konu | Kapsam | Durum |
|---|---|---|
| [Terminal](terminal/) | Dosya gezinme, yol kavramı, temel komutlar | 🟢 Tamamlandı |
| [Python Ortamları](python-ortamlari/) | Kurulum, sanal ortam, bağımlılık | 🟡 1/8 yazıldı |
| [VS Code](vscode/) | Editörü verimli kullanmak | ⚪ Planlandı |
| [Git & GitHub](git-github/) | Sürüm kontrolü, işbirliği, portföy | 🟡 Temeller hazır |
| [Notebook ve Colab](notebook-ve-colab/) | Jupyter, Colab, GPU | ⚪ Planlandı |
| [Yapay Zekâ Asistanları](yapay-zeka-asistanlari/) | LLM ve ajanları doğru kullanma | ⚪ Planlandı |
| [Platformlar](platformlar/) | GPU, Kaggle, Hugging Face | ⚪ Planlandı |

← [Ana sayfa](../README.md)
